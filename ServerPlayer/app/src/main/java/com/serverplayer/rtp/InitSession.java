package com.serverplayer.rtp;

import com.serverplayer.io.Consumer;
import com.serverplayer.jlibrtp.*;
import com.serverplayer.util.IPv4Util;
...
public class InitSession implements RTPAppIntf, RTCPAppIntf, DebugAppIntf {
	 ...
	 * destNetworkAddress 目标ip
	 **/
	public InitSession(Consumer consumer,int localRtpPort, int localRtcpPort, String localIp ,int destRtpPort, int destRtcpPort, String destNetworkAddress) {
		super();
		this.consumer = consumer;
		log.debug("InitSession thread runing");
		DatagramSocket rtpSocket = null;
		...
		//建立会话
		rtpSession = new RTPSession(rtpSocket, rtcpSocket);
		if ( IPv4Util.isIp(localIp) ) {
			this.localIp = IPv4Util.ipToLong(localIp);
			rtpSession.setSsrc(this.localIp);
		}
